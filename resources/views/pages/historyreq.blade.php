...
<div class="max-w-6xl mx-auto p-6 bg-white shadow-md rounded-lg">
    ...
    <!-- Filter Date -->
    <form method="GET" action="{{ route('historyreq.view') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
            <label class="block text-sm text-gray-600 mb-1">Start Date</label>
            <input type="date" name="from" value="{{ request('from') }}" class="w-full border px-3 py-2 rounded-md focus:ring-indigo-500">
        </div>
        <div>
            <label class="block text-sm text-gray-600 mb-1">End Date</label>
            <input type="date" name="to" value="{{ request('to') }}" class="w-full border px-3 py-2 rounded-md focus:ring-indigo-500">
        </div>
    ...
    <div class="overflow-x-auto mb-4">
        <table class="min-w-full bg-white border rounded-md text-sm">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-4 text-gray-500 italic border">No request history found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
